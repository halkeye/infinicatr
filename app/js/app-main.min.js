...
var templates = {
  carousel_entry: require('../templates/carousel_entry.mustache')
};

var $car = $('#catCarousel .carousel-inner');
...
var morePhotosSuccess = function(data) {
  ...
  $.each(data.photos.photo, function(idx, elm) {
    ...
    var $div = $car.append(templates.carousel_entry({ elm: elm }));
...
var carFuncs = {
  next: function() {
    if (!pendingMorePhotos) {
      ...
      alert(config.pending_message); // eslint-disable-line no-alert
    }
  },
  prev: function() {
    $car.carousel('prev');
...
$('#catCarousel-prev').bind('click', function(e) {
  e.preventDefault();
  carFuncs.prev();
});
$('#catCarousel-next').bind('click', function(e) {
  e.preventDefault();
  carFuncs.next();
});
